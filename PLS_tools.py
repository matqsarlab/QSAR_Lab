...
def latent_vectors(model, X_train, n_comp, *args, **kwargs):
    ...
    loadings = pd.DataFrame(model.x_loadings_, columns=latent_no, index=X_train.columns)
    ...
    if "title" in kwargs.keys():
        plt.suptitle(kwargs["title"], fontsize=22)
    plt.tight_layout()
    # plt.show()
...
def distribution_in_LVs(model, X_train, y_train, X_test, y_test, n_comp, **kwargs):
    ...
    labels = None
    n = coeff.shape[0]
    scalex = 1.0 / (np.max([xs.max(), xsv.max()]) - np.min([xs.min(), xsv.min()]))
    scaley = 1.0 / (np.max([ys.max(), ysv.max()]) - np.min([ys.min(), ysv.min()]))
    ...
    if "annotate" in kwargs.keys() and kwargs["annotate"]:
        ...
        for i, txt in enumerate(X_train.index):
            t = plt.text(
                scalex * xs[i], scaley * ys[i], txt[: txt.index(")")], fontsize=24
            )
        ...
        if "adjust_text" in kwargs.keys() and kwargs["adjust_text"]:
            adjust_text(
                texts_train,
                arrowprops=dict(arrowstyle="->", color="r", lw=1.5, alpha=0.3),
            )
            # adjust_text(
            #     texts_test,
            #     arrowprops=dict(arrowstyle="->", color="r", lw=1.5, alpha=0.3),
    ...
    if "cbar_limit" in kwargs.keys():
        plt.clim(kwargs["cbar_limit"])
